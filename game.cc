...
// mv_pos.x - 0.4 * cos((theta - 30) * M_PI/180), agency_loc.y + 0.75, mv_pos.z - 0.4 * sin((theta - 30) * M_PI/180)
  ...
  // Pickups
  if (!get_sweeper) vaccuum_pickup(); else ; // vaccuum();
  if (!get_key) key();
...
void floor() {
  object(mv, model_view, -50.0 + FLOOR_SIZE / 2, -2.5, -50.0 + FLOOR_SIZE / 2, ROOF_W, ROOF_H, ROOF_D, ROOF_R, ROOF_G, ROOF_B, 0, 0, 0, 0, 0, 0);
...
void vaccuum_pickup() {
  object(pv, model_view, -5.0 + 0.0*0.2, 0.0 - 3.0*0.2, -45.0 + 0.0*0.2, BODYL_W*0.2, BODYL_H*0.2, BODYL_D*0.2, VACCUUM_R, VACCUUM_G, VACCUUM_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 0.0*0.2, 0.0 + 0.5*0.2, -45.0 - 0.5*0.2, BODYS_W*0.2, BODYS_H*0.2, BODYS_D*0.2, VACCUUM_R, VACCUUM_G, VACCUUM_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 2.5*0.2, 0.0 + 0.5*0.2, -45.0 + 1.0*0.2, HOSEL_W*0.203, HOSEL_H*0.2, HOSEL_D*0.203, HOSE_R, HOSE_G, HOSE_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 0.0*0.2, 0.0 + 2.5*0.2, -45.0 + 1.0*0.2, HOSEM_W*0.201, HOSEM_H*0.2, HOSEM_D*0.201, HOSE_R, HOSE_G, HOSE_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 1.25*0.2, 0.0 + 3.5*0.2, -45.0 + 1.0*0.2, HOSES_W*0.202, HOSES_H*0.2, HOSES_D*0.202, HOSE_R, HOSE_G, HOSE_B, 0, disp_rot, 90, 0, 0, 0);
  object(pv, model_view, -5.0 + 2.5*0.2, 0.0 - 2.5*0.2, -45.0 + 1.5*0.2, NOZZLE_W*0.2, NOZZLE_H*0.2, NOZZLE_D*0.2, HOSE_R, HOSE_G, HOSE_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 - 1.0*0.2, 0.0 - 1.5*0.2, -45.0 - 2.0*0.2, STRAP_W*0.2, STRAP_H*0.2, STRAP_D*0.2, STRAP_R, STRAP_G, STRAP_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 1.0*0.2, 0.0 - 1.5*0.2, -45.0 - 2.0*0.2, STRAP_W*0.2, STRAP_H*0.2, STRAP_D*0.2, STRAP_R, STRAP_G, STRAP_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 - 2.5*0.2, 0.0 - 3.5*0.2, -45.0 - 0.5*0.2, WHEEL_W*0.2, WHEEL_H*0.2, WHEEL_D*0.2, WHEEL_R, WHEEL_G, WHEEL_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 2.5*0.2, 0.0 - 3.5*0.2, -45.0 - 0.5*0.2, WHEEL_W*0.2, WHEEL_H*0.2, WHEEL_D*0.2, WHEEL_R, WHEEL_G, WHEEL_B, 0, disp_rot, 0, 0, 0, 0);
  object(pv, model_view, -5.0 + 0.0*0.2, 0.0 + 0.5*0.2, -45.0 + 1.0*0.2, LIGHT_W*0.2, LIGHT_H*0.2, LIGHT_D*0.2, LIGHT_R, LIGHT_G, LIGHT_B, 0, disp_rot, 0, 0, 0, 0);
}
...
void gun() {
  object(pv, model_view, mv_pos.x - 5.0 * cos((theta - 27) * M_PI/180) * cos(phi * M_PI/180)*0.2, mv_pos.y - 5.0 * sin(phi * M_PI/180)*0.2, mv_pos.z - 5.0 * sin((theta - 27) * M_PI/180) * cos(phi * M_PI/180)*0.2, BARREL_W*0.2, BARREL_H*0.2, BARREL_D*0.2, BARREL_R, BARREL_G, BARREL_B, 0, -theta + 90, 0, 0, 0, 0);
  object(pv, model_view, mv_pos.x - 5.75 * cos((theta - 20) * M_PI/180) * cos(phi * M_PI/180)*0.2, mv_pos.y - 5.75 * sin(phi * M_PI/180)*0.2, mv_pos.z - 5.75 * sin((theta - 20) * M_PI/180) * cos(phi * M_PI/180)*0.2, SIGHT_W*0.2, SIGHT_H*0.2, SIGHT_D*0.2, SIGHT_R, SIGHT_G, SIGHT_B, 0, -theta + 90, 0, 0, 0, 0);
  object(pv, model_view, mv_pos.x - 4.0 * cos((theta - 35) * M_PI/180) * cos(phi * M_PI/180)*0.2, mv_pos.y - 4.0 * sin(phi * M_PI/180)*0.2, mv_pos.z - 4.0 * sin((theta - 35) * M_PI/180) * cos(phi * M_PI/180)*0.2, HANDLE_W*0.2, HANDLE_H*0.2, HANDLE_D*0.2, HANDLE_R, HANDLE_G, HANDLE_B, 0, -theta + 90, 0, 0, 0, 0);
}
